...
void uart_init(void)
{
	...
	SCON=0x40;			//Configure UART peripheral for 8 transfer 
	TR1=1;						//Start Timer 1
...
{
	SBUF=ch;				//Load  in SBUF
	while(!tx_complete); 			//Wait for tx_complete flag  to complete)
...
{
	unsigned char ch = 0;
	while(!rx_complete);				//Wait for rx_complete to complete)
	rx_complete = 0;
	ch = SBUF;					//Read from SBUF
...
void serial_ISR(void) interrupt 4
{
		...
		{
			TI = 0;			//Clear TI flag
			tx_complete = 1;	//Set tx_complete flag indicating completion
		}
		else if(RI==1)			//check whether RI is set
		{
			RI = 0;			//Clear RI flag
			rx_complete = 1;	//Set rx_complete flag indicating completion
		}
}
